#UNIFIED_V1
"""
Unified agent combining the best elements of all three prior agents:

Agent 4 (GAEL_TUNED original):
  + obs_likelihood_history (temporal Bayesian smoothing with T2)
  + _corridor_profile (run-length analysis)
  + _setup_plan_profile (2-ply lookahead for chain planning)
  + position_history / novelty_bonus / anti_loop_eval
  + failed_search_cooldown + last_failed_search_loc

Agent 5 (GAEL_TUNED carpet-fixed):
  + _carpet_profile sort fix (gain DESC, roll DESC)
  + roll=1 penalty (-40) in _order_moves
  + _has_strong_scoring_move: ignores roll<2
  + Search EV formula: 6P-2 with 10x scaling

Agent 6 (CARPET_OPTIMIZED_V2):
  + _carpet_is_acceptable() hard gate (never roll=1, no early roll=2 if ≥6 coming)
  + _count_primed_chain() (exact chain geometry)
  + _carpet_potential_score() (convex payoff lookahead)
  + _best_carpet_in_two_moves() (2-ply carpet ceiling)
  + game-phase-aware thresholds (urgency scaling in _valid_search)
  + _is_endgame / _is_desperate
  + phase_multiplier in _evaluate (lead worth more late game)
  + two_move carpet bonus in _evaluate

NEW IMPROVEMENTS:
  + Unified _carpet_profile uses both actual gain AND chain geometry
  + Prime scoring uses chain-extension value (not just future carpet gain)
  + Search threshold: continuous urgency decay, not step function
  + _order_moves: carpet, prime, search, plain all scored on unified scale
  + _evaluate: carpet_potential_score weighted by phase
  + Expectiminimax: wider branching (8 moves) + separate chance-node
    belief propagation respects T2 for 2-step look-ahead
  + Anti-loop now also penalises oscillation between 2 cells
"""
